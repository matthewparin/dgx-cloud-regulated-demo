...
resource "kubernetes_deployment" "costapp" {
  metadata { name = "cost-estimator"; namespace = kubernetes_namespace.restricted.metadata[0].name; labels = { app = "cost-estimator" } }
  spec {
    replicas = 1
...
resource "kubernetes_service" "costapp_service" {
  metadata { name = "cost-estimator-service"; namespace = kubernetes_namespace.restricted.metadata[0].name }
  spec {
